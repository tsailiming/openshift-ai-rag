...
from pymilvus import utility, connections, CollectionSchema, FieldSchema, DataType
from langchain_community.document_loaders import TextLoader
import PyPDF2  # You might need to install this for PDF support
from common import load_embedding_model, create_collection_if_not_exists, ingest_text, perform_similarity_search
from filteredretrieval import FilteredRetriever
...
def create_milvus_collection(new_collection_name: str, 
                             milvus_host: str, 
                             milvus_port: str, 
                             milvus_username: str, 
                             milvus_password: str):
    create_collection_if_not_exists(new_collection_name)

# Get default values of environment variables
#milvus_host = os.environ.get("MILVUS_HOST", "ab5a452bd9b654e8094493049fb1607b-978231131.ap-southeast-1.elb.amazonaws.com")
...
model_endpoints = {
    "vLLM": os.getenv("VLLM_ENDPOINT", "https://granite-demo.apps.multus-test.sandbox323.opentlc.com/v1"),
...
try:
    ...
    if not milvus_collections:
        milvus_collections = ["default"]
...
def upload_and_process_file(uploaded_file):
    ...
    if uploaded_file.type == "text/plain":
        # Handle text files
        return uploaded_file.read().decode("utf-8"), 'txt', metadata
        ...
        text = ""
        metadata["num_pages"] = len(pdf_reader.pages)  # Store the number of pages
        for page in pdf_reader.pages:
            text += page.extract_text() or ""  # Extract text from each page
    
        return text, 'txt', metadata
    elif file_extension == "md" or uploaded_file.type == "application/octet-stream":
        # Handle Markdown files
        return uploaded_file.read().decode("utf-8"), 'md', metadata
...
# def ingest_embeddings(text: str, selected_collection: str, metadata: Dict[str, Any]):
...
#         st.success("Successfully ingested embeddings into Milvus.")
#     else:
#         st.error("No text to ingest.")


if uploaded_file is not None:
    # Process the uploaded file
    text_content, file_type, file_metadata = upload_and_process_file(uploaded_file)
    
    if text_content:
        st.text_area("Extracted Text", text_content, height=300)
        
        # Button to ingest embeddings
        if st.button("Ingest into Milvus"):
            create_collection_if_not_exists(milvus_collection)
...
def stream(input_text: str, selected_collection: str, model_url_key: str) -> Generator:
    ...
    docs = perform_similarity_search(input_text, 
                                     selected_collection,
                                     milvus_host=milvus_host,
                                     milvus_port=milvus_port,
                                     milvus_username=milvus_username,
                                     milvus_password=milvus_password)                                     
    filtered_retriever = FilteredRetriever(filtered_docs=[doc for doc, _ in docs])
    ...
    #         search_kwargs={"k": int(max_retrieved_docs),
